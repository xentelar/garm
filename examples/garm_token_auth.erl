...
start(DomainKey, SecScheme , SecurityDef) ->
	?LOG_INFO(#{description => "Start Token validator", 
...
is_authorized(Req, _SecuritySchema)  ->
	case garm_http_request:get_header_value(<<"Token">>, Req) of
		undefined ->
			?LOG_DEBUG(#{description => "Token header is undefined"}),
			false;
		Token ->
